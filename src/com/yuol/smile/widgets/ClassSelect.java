		...
		new Thread(){
			...
			public void run(){
				try {
					...
					try {
						msg.obj= Jsoup.connect(Api.Jwc.getJwcClass()).data(getParams("selDepart",current_class,current_term,current_college,"174837","学生")).timeout(20 * 1000).post();
					} catch (IOException e) {
						System.out.println("查询班级出错!");
						e.printStackTrace();
					}
					/*Message msg=new Message();
